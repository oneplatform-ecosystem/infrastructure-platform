...
variable "location" {
  description = "The Azure Region where the Azure Content Safety should be created"
  type        = string
}

variable "resource_group_name" {
  description = "The name of the resource group in which to create the Azure Content Safety"
  type        = string
}

# Azure Content Safety Configuration
variable "sku_name" {
  description = "The SKU name for the Azure Content Safety. Possible values are F0 (Free), S0 (Standard)"
  type        = string
  default     = "S0"
  validation {
    condition     = contains(["F0", "S0"], var.sku_name)
    error_message = "The sku_name must be F0 or S0 for Azure Content Safety."
  }
}

variable "custom_subdomain_name" {
  description = "The custom subdomain name for the Azure Content Safety. If not provided, it will use the generated label ID"
  type        = string
  default     = null
}

variable "public_network_access_enabled" {
  description = "Whether public network access is allowed for the Azure Content Safety"
  type        = bool
  default     = false
}
...
variable "network_acls" {
  description = "Network rules restricting access to the Azure Content Safety"
  type = object({
...
variable "identity_type" {
  description = "The type of Managed Identity which should be assigned to the Azure Content Safety. Possible values are SystemAssigned, UserAssigned and SystemAssigned, UserAssigned"
  type        = string
  default     = null
  validation {
...
variable "identity_ids" {
  description = "A list of User Assigned Managed Identity IDs to be assigned to this Azure Content Safety. Required when identity_type is UserAssigned or SystemAssigned, UserAssigned"
  type        = list(string)
  default     = null
}
